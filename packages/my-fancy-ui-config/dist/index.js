...
module.exports = createConfig({
  ...
  loggerId: '@immowelt/my-fancy-ui-config',
  pathsByKey: {
  ...
  requiredVariablesByKey: {
    ...
    MY_FANCY_UI_PORT: num({
      default: 8080,
      desc: 'The HTTP port of the webserver that will serve the application to the user.'
    }),
  ...
  extend: config => ({
    ...
    server: {
      url: `http://${config.env.MY_FANCY_UI_HOST}:${config.env.MY_FANCY_UI_PORT}/`
    },
    hypernova: {
      url: `http://${config.env.MY_FANCY_UI_HOST}:${config.env.MY_FANCY_UI_SSR_PORT}/`,
      endpoint: 'batch'
    },
